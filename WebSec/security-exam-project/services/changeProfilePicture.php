...
if ($referer !== "https://188.226.141.57/views/profile.php"){
    ...
    $filetype = $_FILES['image']['type']; //returns the mimetype

    $allowed = array("image/jpeg", "image/gif");
    if(!in_array($filetype, $allowed)) {
        $error_message = 'Only jpeg and gif files are allowed.';
        header("Location: ../views/profile.php");
    }else{
        //file sanitization
        $notHashedSanitizedFilename = preg_replace(array('/\s/', '/\.[\.]+/', '/[^\w_\.\-]/'), array('_', '.', ''), $filename);

        $sanitizedFilename = hash('sha256', $notHashedSanitizedFilename);
